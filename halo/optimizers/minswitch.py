...
def build_min_switch_plan(
    ...
    *,
    ...
    """Cache-agnostic, model-switch-minimizing baseline."""
    worker_ids = tuple(sorted(workers))
    if not worker_ids:
        raise ValueError("MinSwitch scheduler requires at least one worker.")
    ...
    for idx, node_id in enumerate(order):
        ...
        if node.engine == "vllm":
            ...
            if same:
                ...
                empty = [wid for wid in eligible if last_model.get(wid) is None]
                if empty:
                    chosen = min(empty, key=lambda wid: (load.get(wid, 0), wid))

        if chosen is None:
            start = idx % total_workers
            for offset in range(total_workers):
                cand = worker_ids[(start + offset) % total_workers]
                if cand in eligible:
                    chosen = cand
                    break
        if chosen is None:
